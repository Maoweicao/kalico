...
```
klippy/mcu.py          â”€â”€ Configuration, connect/disconnect logic
klippy/serialhdl.py    â”€â”€ Transport adapters (UART, CAN, TCP, UDP, Pipe, File)
klippy/chelper/serialqueue.c â”€â”€ Low-level I/O, retransmit, clock tracking
```
...
```
MCU (STM32/AVR/...) â”€â”€UARTâ”€â”€â–?ESP32/RPi (Bridge) â”€â”€TCPâ”€â”€â–?Klippy
```
...
1. `tcp_host` is set â†?**TCP**
2. `udp_host` is set â†?**UDP**
3. `canbus_uuid` is set â†?**CAN Bus**
4. `serial` is set, starts with `/dev/rpmsg_` or `/tmp/klipper_host_`
   â†?**Pipe / PTY**
5. `serial` is set, other path â†?**UART / Serial**
