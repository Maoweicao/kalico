...
- **The `plugins/` directory is git-untracked.** It does not exist in the
  upstream Kalico tree. Drop files here freely â€?no dirty git tree.
...
Restart Kalico. Run `MY_COMMAND` from the console â€?you should see "Hello from my_tool!".
...
```
klippy/
â”œâ”€â”€ extras/                  # Built-in modules (part of Kalico core)
â”?  â”œâ”€â”€ __init__.py
â”?  â”œâ”€â”€ respond.py
â”?  â””â”€â”€ ...
â”œâ”€â”€ plugins/                 # User plugins (git-untracked)
â”?  â”œâ”€â”€ __init__.py          # Package marker (always present)
â”?  â”œâ”€â”€ my_tool.py           # Single-file plugin
â”?  â”œâ”€â”€ my_complex_plugin/   # Sub-package plugin
â”?  â”?  â”œâ”€â”€ __init__.py      # Contains load_config / load_config_prefix
...
- The **Printer** acts as the **Service Locator** â€?your plugin pulls dependencies by name:
  - `printer.lookup_object("gcode")` â€?get a registered service
  - `printer.load_object(config, "heaters")` â€?lazily load another module
  - `printer.register_event_handler("klippy:ready", cb)` â€?subscribe to events
  - `printer.lookup_components("load_cell_sensors")` â€?query a subsystem registry
...
**Phase 1 â€?Discovery** (`printer.py:_load_modules`)
...
**Phase 2 â€?Loading** (`printer.py:_load_modules`)
Each `PrinterModule.load()` calls `importlib.import_module(module_info.name)`.
Exceptions during import are caught and stored â€?the error is raised later,
...
**Phase 3 â€?Component Registration** (`printer.py:_register_subsystem_components`)
...
**Phase 4 â€?Config Initialization** (`printer.py:_read_config`)
...
**Phase 5 â€?Connect** (`printer.py:_connect`)
...
**Phase 6 â€?Ready** (`printer.py:_connect`)
...
**Phase 7 â€?Shutdown / Restart** (`printer.py:run`)
...
There is no hot-plug mechanism â€?a restart is required.
...
### `load_config_prefix(config)` â†?object
...
```python
sensors = printer.lookup_components("my_subsystem")  # â†?{"my_driver_v1": MyDriverClass, ...}
...
- The initialization order is non-trivial â€?not all modules exist when your
...
or `None`. Lists and dicts must be treated as immutable â€?return a new object
if contents change.
...
2. **Assign all member variables in `__init__`.** Avoid dynamically creating
   attributes â€?use `self.xyz = None` in the constructor.
...
Fields explained:
- **source**: `"plugins"` or `"extras"` â€?where the module came from
...
1. **Move the file.** Copy `klippy/extras/my_module.py` â†?`klippy/plugins/my_module.py`.
...
2. Scans for plugin files â€?any `.py` file containing `load_config` or
   `load_config_prefix`.
...
========================================
  ...
  Available G-code commands:
    COOL_CALIBRATE    â†?calibrate the cool sensor
    COOL_REPORT       â†?report current readings
...
- The file name matches exactly (e.g., `my_tool.py` â†?`[my_tool]`)
...
- Restart Kalico (a `RESTART` is required â€?dynamic reloading is not supported)
