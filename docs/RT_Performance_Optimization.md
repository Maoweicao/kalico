...
G-code (Python, ~ms per command)
  ...
  -> trapq_append (C) — trapezoidal velocity queue
  -> itersolve (C) — secant-method iterative step-time solver
  -> stepcompress (C) — step pulse compression/queuing
  -> serialqueue (C, background thread) — scheduled serial/CAN TX to MCU
  -> UART/CAN hardware
```
...
- `epoll_create1` / `epoll_wait` — O(1) event-driven dispatch
- `timerfd_create` / `timerfd_settime` — nanosecond-level timer precision
...
inverted — the Python thread runs at low priority, preventing the mutex from being
released quickly.
