...
class FieldHelper:
    ...
    def __init__(self, all_fields, signed_fields=None, field_formatters=None):
        self.all_fields = all_fields
        self.signed_fields = set(signed_fields or [])
    ...
    def set_config_field(self, config, field_name, default):
        ...
        if maxval == 1:
            ...
            val = config.getint(
                config_name, default, minval=-32768, maxval=32767
            )
        else:
...
class LYXCurrentHelper:
    ...
    def __init__(self, config, mcu_lyx):
        ...
        self.sense_resistor = config.getfloat(
            "sense_resistor", 0.050, above=0.0
        )
        run_current = config.getfloat("run_current", 1.4, above=0.0)
        hold_current = config.getfloat("hold_current", None, above=0.0)
...
class LYXCommandHelper:
    ...
    def __init__(self, config, mcu_lyx, current_helper, micro_helper):
        ...
        gcode.register_mux_command(
            "SET_LYX_FIELD", "STEPPER", self.name, self.cmd_SET_LYX_FIELD
        )
        gcode.register_mux_command(
            "SET_LYX_CURRENT", "STEPPER", self.name, self.cmd_SET_LYX_CURRENT
        )
        gcode.register_mux_command(
            "SET_LYX_MICROSTEP",
            "STEPPER",
    ...
    def cmd_SET_LYX_CURRENT(self, gcmd):
        ...
        run = gcmd.get_float("CURRENT", None, minval=0.0, maxval=max_cur)
        hold = gcmd.get_float("HOLDCURRENT", None, above=0.0, maxval=max_cur)
    ...
    def cmd_SET_LYX_MICROSTEP(self, gcmd):
        ...
        self.mcu_lyx.set_register(
            reg_name, self.fields.get_field("microstep_ratio"), print_time
        )
        gcmd.respond_info(
            f"Set microstep = {microstep}, "
    ...
    def setup_register_dump(self, read_registers):
        ...
        gcode = self.printer.lookup_object("gcode")
        gcode.register_mux_command(
            "DUMP_LYX", "STEPPER", self.name, self.cmd_DUMP_LYX
        )
