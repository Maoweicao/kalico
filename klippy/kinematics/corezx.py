# Code for handling the kinematics of corezx robots
#
# CoreZX: X moves independently, Z and Y are coupled
...
class CoreZXKinematics:
    def __init__(self, toolhead, config):
        ...
        self.max_y_velocity = config.getfloat(
            "max_y_velocity", max_velocity, above=0.0, maxval=max_velocity
        )
        self.max_y_accel = config.getfloat(
            "max_y_accel", max_accel, above=0.0, maxval=max_accel
        )
    ...
    def calc_position(self, stepper_positions):
        ...
        # CoreZX: A=X+Z, B=X-Z, C=Y
        # X = (A+B)/2, Y = C, Z = (A-B)/2
        return [0.5 * (pos[0] + pos[1]), pos[2], 0.5 * (pos[0] - pos[1])]
    ...
    def check_move(self, move):
        ...
        if not move.axes_d[2]:
            return
        self._check_endstops(move)
        y_ratio = move.move_d / abs(move.axes_d[2])
        move.limit_speed(
...
def load_kinematics(toolhead, config):
    return CoreZXKinematics(toolhead, config)
